...
export default class GameList extends Component {
  ...
  render() {
    if (this.state.joinedGame) {
      return (
        <GameItem gamename={this.state.gamename} game={this.state.gameinfo} />
      );
      ...
      return (
        <Card>
          ...
          <Card.Text>
            {"Please delete your game before joining someone else's."}
          </Card.Text>
        </Card>
      );
    }
    return (
      <CardColumns>
        {this.state.gamenames.map((gamename, i) => {
          return (
            <GameItem gamename={gamename} game={this.state.games[gamename]} />
          );
        })}
      </CardColumns>
    );
